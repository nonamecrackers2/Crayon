	...
	public static void onPhysicsStep(PhysicsSpaceEvent.Step event)
	{
		...
		for (var rigidBody : space.getRigidBodiesByClass(ElementRigidBody.class))
		{
			...
			for (var triangle : triangles)
			{
				...
				final var blockPos = new BlockPos((int) (location.x + centroid.x), (int) (location.y + centroid.y), (int) (location.z + centroid.z));
			...
			for (var triangle : triangles)
			{
				...
				if (submergedTriangles.contains(triangle))
				{
					...
					final var waterHeight = chunkCache.getFluidColumn(blockPos).map(fluidColumn -> (float) fluidColumn.getTop().blockPos().getY() + fluidColumn.getTopHeight(posRelativeToBlockCenter) - location.y - centroid.y).orElse(0.0f);
					...
					if (rigidBody.isWaterDragEnabled())
					{
						...
						if (Math.signum(netVelocity.dot(area)) == 1)
						{
							...
							/*
							 * This stopping force is how we prevent objects from entering orbit upon
							 * touching water :(
							 */
...
//                            }
					...
					{
						/*
						 * air_density_at_sea_level * e^(gravity * molar_mass_of_air * sea_level /
